...
public class NGramIndexWorker implements OrderedValuesIndex, QNamedKeysIndex {
    ...
    private void saveIndex() {
        ...
        for (Map.Entry<QNameTerm, OccurrenceList> entry : ngrams.entrySet()) {
            ...
            final Lock writeLock = index.db.getLock().writeLock(); //TODO(AR) should happen outside for-loop
            writeLock.lock();
            try {
                ...
                writeLock.unlock();
                os.clear(); //TODO(AR) should happen before unlock
            }
        }
        ngrams.clear();
    }
    ...
    private void dropIndex(ReindexMode mode) {
        ...
        for (Map.Entry<QNameTerm, OccurrenceList> entry : ngrams.entrySet()) {
            ...
            final Lock writeLock = index.db.getLock().writeLock();
            writeLock.lock(); //TODO(AR) should happen outside for-loop
            try {
                NGramQNameKey value = new NGramQNameKey(currentDoc.getCollection().getId(), key.qname,
                        index.getBrokerPool().getSymbols(), key.term);
        ...
        ngrams.clear(); //TODO(AR) should maybe happen before unlock?
    }
 ...
 throws XPathException {
        ...
        for (Iterator<org.exist.collections.Collection> iter = docs.getCollectionIterator(); iter.hasNext();) {
            ...
            for (int i = 0; i < qnames.size(); i++) {
                ...
                final Lock readLock = index.db.getLock().readLock(); //TODO(AR) should happen outside for-loop
                readLock.lock();
                try {
    ...
    public Occurrences[] scanIndex(XQueryContext context, DocumentSet docs, NodeSet contextSet, Map hints) {
        ...
        final Lock readLock = index.db.getLock().readLock();
        ...
        for (int q = 0; q < qnames.size(); q++) {
            for (Iterator<Collection> i = docs.getCollectionIterator(); i.hasNext();) {
                ...
                readLock.lock(); //TODO(AR) should happen outside for loop
                try {
